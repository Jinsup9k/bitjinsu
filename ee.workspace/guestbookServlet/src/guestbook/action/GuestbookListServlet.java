	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (list != null) {
			for (int i = 1; i <= totalP; i++) {
				if (i == pg)
					out.println(
							"[<a id='currentPaging' href='http://localhost:8080/guestbookServlet/GuestbookListServlet?pg="
									+ i + "'>" + i + "</a>]");
				else
					out.println("[<a id='paging' href='http://localhost:8080/guestbookServlet/GuestbookListServlet?pg="
							+ i + "'>" + i + "</a>]");
			}
